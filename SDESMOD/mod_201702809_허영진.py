...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    temp = []
    result = bitarray()
    #divide text 8 bits and push to array
    for i in range(len(text)//8):
        temp.append(text[8*i:8*(i+1)])
        
    #sdes such texts and connect
    for i in range(len(temp)):
        temp[i] = sdes(temp[i], key, MODE_ENCRYPT)
...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    ...
    for i in range(len(ciphertext)//8):
        temp.append(ciphertext[8*i:8*(i+1)])
        
    for i in range(len(temp)):
        temp[i] = sdes(temp[i], key, MODE_DECRYPT)
        result.extend(temp[i])
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(len(temp)):
        if i == 0:
            temp[i] = sdes((temp[i]^iv), key, MODE_ENCRYPT)
        else:
            temp[i] = sdes((temp[i-1] ^ temp[i]), key, MODE_ENCRYPT)
        result.extend(temp[i])
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    temp = [] #for result
    temp2 = [] #splited ciphertext
    result = bitarray()
    
    #split and push to temp2
    for i in range(len(ciphertext)//8):
        temp2.append(ciphertext[8*i:8*(i+1)])
        
    for i in range(len(temp2)):
        if i == 0:
            temp.append(sdes(temp2[i], key, MODE_DECRYPT) ^ iv)
        else:
            temp.append(sdes(temp2[i], key, MODE_DECRYPT) ^ temp2[i-1])
        result.extend(temp[i])
